...
<head>
	...
	<title>TRAM RECRUITMENT COMPANY | New Bookings   </title>
...
<div id="print">
								<table border="1"  class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%"  >
									...
									<?php 
$bid=intval($_GET['bid']);
									$sql = "SELECT tblusers.*,tblcategory.CategoryName,tblmaids.names,tblbooking.FromDate,tblbooking.ToDate,tblbooking.message,tblbooking.maidId as vid,tblbooking.Status,tblbooking.PostingDate,tblbooking.id,tblbooking.BookingNumber,
DATEDIFF(tblbooking.ToDate,tblbooking.FromDate) as totalnodays,tblmaids.address
									  from tblbooking join tblmaids on tblmaids.id=tblbooking.maidId join tblusers on tblusers.EmailId=tblbooking.userEmail join tblcategory on tblmaids.categoryname=tblcategory.id where tblbooking.id=:bid";
$query = $dbh -> prepare($sql);
	...
	<h3 style="text-align:center; color:red">#<?php echo htmlentities($result->BookingNumber);?> Booking Details </h3>
		...
		<tr>
										...
										<tr>
											...
											<tr>											
											<th>Domestic Worker's Name</th>
											<td><a href="edit-maids.php?id=<?php echo htmlentities($result->vid);?>"><?php echo htmlentities($result->names);?></td>
											<th>Category</th>
